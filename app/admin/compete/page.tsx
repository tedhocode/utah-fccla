...
export default function AdminCompetePage() {
  ...
  async function loadDocuments() {
    try {
      ...
      // Filter to competition-related documents
      const filtered = data.filter((doc: Document) =>
        ['star_events', 'culinary', 'competition'].includes(doc.category)
      )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await upsertDocument({
        ...
        category: formData.category || 'competition',
  ...
  return (
    <div className="p-8 text-white">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white font-outfit">Compete</h1>
        <p className="text-gray-400 mt-1">Manage competition-related documents. For general downloads, go to Resources.</p>
      </div>
  ...
  return (
    <div className="bg-gray-900 border-2 border-fccla-red/30 rounded-xl p-6 mb-6">
      <h3 className="text-lg font-semibold mb-4">
        {isEditing ? 'Edit Document' : 'Add Competition Document'}
      </h3>
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        >
          ...
          <option value="star_events">Star Events</option>
